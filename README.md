# ТИМП — Подгруппа 5 (ветка `lan-demo`)
...
> **Эта ветка (`lan-demo`)** предназначена для демонстрации на защите:
> сервер запускается на одном компьютере, клиент — на другом, оба в одной локальной сети.
> Ветка `main` для этого не трогается.

---

## 🎓 Демонстрация в локальной сети (для защиты)

Сервер слушает на всех интерфейсах (`0.0.0.0:33333`), клиент берёт адрес сервера
из переменной окружения `TIMP_SERVER_HOST` / `TIMP_SERVER_PORT` или из файла `server.conf`.
...
Скрипт соберёт сервер, запустит его и **покажет IP-адрес** этого компьютера в сети
(например `192.168.1.50`). Запомните этот адрес — он понадобится на клиенте.

### Компьютер №2 — КЛИЕНТ

```bash
./run-client.sh 192.168.1.50        # подставьте IP сервера из шага выше
# при другом порте:  ./run-client.sh 192.168.1.50 33333
```

Скрипт соберёт клиент, проверит доступность сервера и запустит GUI, уже настроенный
на указанный адрес. Без аргумента (`./run-client.sh`) клиент подключается к `127.0.0.1`
(локальный запуск на одном ПК).

> Альтернатива скрипту: скопировать `server.conf.example` в `server.conf`,
> вписать туда `host=<IP сервера>` и запустить клиент обычным способом.

### ⚠️ Что нужно сделать ВАМ вручную (на защите)

Эти шаги зависят от конкретных компьютеров в аудитории и не могут быть выполнены заранее:

1. **Оба компьютера в одной локальной сети.** Подключить оба ПК к одному Wi-Fi/коммутатору.
   Проверить, что они «видят» друг друга:
   ```bash
   # на клиенте, подставив IP сервера:
   ping 192.168.1.50
   ```
2. **Узнать IP сервера.** Его печатает `./run-server.sh`. Вручную можно так:
   ```bash
   ip -4 addr show | grep inet        # Linux
   ```
3. **Открыть порт 33333 в firewall на компьютере-сервере** (если включён):
   ```bash
   # Linux (firewalld):
   sudo firewall-cmd --add-port=33333/tcp
   # Linux (ufw):
   sudo ufw allow 33333/tcp
   # Linux (iptables):
   sudo iptables -A INPUT -p tcp --dport 33333 -j ACCEPT
   ```
4. **Настроить `server/email.txt`** на компьютере-сервере (см. раздел ниже) —
   нужно для отправки кодов подтверждения на почту.
5. Если сеть с изоляцией клиентов (гостевой Wi-Fi) — соединение между ПК может
   блокироваться. Использовать проводную сеть или точку доступа без изоляции.

---

## Быстрый запуск

Одна команда — и всё работает: сервер поднимается на порту `33333`, собирается клиент и открывается GUI:
...
1. Останавливает старые экземпляры сервера/клиента (если были).
2. Пересобирает сервер и клиент (qmake + make).
3. Запускает сервер в фоне и ждёт, пока он будет готов.
4. Запускает GUI-клиент.
5. При нажатии `Ctrl+C` корректно останавливает оба процесса.
