...
void VerifyWidget::onVerifyResponseReceived(const QString &response)
{
    ...
    // Игнорируем служебные ответы от auth-запроса (fire-and-forget из AuthWidget)
    if (r == "auth_code_sent" || r == "auth-" || r.startsWith("auth-||"))
        return;

    if (!m_waitingForVerify) return;
    m_waitingForVerify = false;

    // Успешная аутентификация: сервер отвечает "auth+||<login>"
    ...
    // Неверный код: сервер отвечает "auth-||wrong_code"
    if (r.startsWith("auth-||")) {
        lockLevel++;
        if (lockLevel >= 3) {
            isLocked = true;
    ...
    // Неизвестный ответ — общая ошибка
